...
def time(nodepoint): #노드에서 다음 노드로 갈 때의 거리와 높이를 시간으로 변환해주는 함수
    time=math.sqrt(pow(nodepoint[1],2)+pow(nodepoint[2]*cons1,2))*(cons2*nodepoint[2])
    nodepoint[1]=time
    nodepoint[2]=0
...
def findmin(node): #현재 노드에서 다음 노드로 갈 때, 다음 노드들 중 걸리는 시간 중 최소 시간을 구하는 함수, 반환값: node이름
    ...
    for i in next_node: #nodes=[[a,100],[b,20],[c,300]]
        if mintime==-1:
            ...
            if do[i][1]<mintime and do[i][1]!=-1: #갈 수 있는 노드들 중에서, 시간이 가장 짧은 노드 발견
                mintime=do[i][1]
                min_name=i
        return min_name

def search(stops, end, root, tim=0): #전체 소요 시간을 구하는 함수
    ...
    for j in do[stops][2]: #갈 수 있는 다음 노드들(do[stops][2])을 리스트에서 찾아본 후, 걸리는 시간이 초기값 -1이면 지금 값으로 갱신
        if do[j[1]]==-1:
            do[j[1]]=j[1]
    ...
    search(next,end,tim+do[next][1])


print(search('DormE','DormF',[]))
